...
sessions = {}  # {session_id: {user_id, expires_at, csrf_token}}
...
def init_database():
    ...
    with closing(sqlite3.connect(DB_FILE)) as conn:
        ...
        # Migration: Add websocket and header-related columns
        try:
            cursor.execute("ALTER TABLE reverse_proxies ADD COLUMN websocket INTEGER DEFAULT 0")
            logger.info("Added websocket column to reverse_proxies table")
        except sqlite3.OperationalError:
            pass

        try:
            cursor.execute("ALTER TABLE reverse_proxies ADD COLUMN header_up_host TEXT")
            logger.info("Added header_up_host column to reverse_proxies table")
        except sqlite3.OperationalError:
            pass

        try:
            cursor.execute("ALTER TABLE reverse_proxies ADD COLUMN remove_origin INTEGER DEFAULT 0")
            logger.info("Added remove_origin column to reverse_proxies table")
        except sqlite3.OperationalError:
            pass

        try:
            cursor.execute("ALTER TABLE reverse_proxies ADD COLUMN remove_referer INTEGER DEFAULT 0")
            logger.info("Added remove_referer column to reverse_proxies table")
        except sqlite3.OperationalError:
            pass

        try:
            cursor.execute("ALTER TABLE reverse_proxies ADD COLUMN custom_headers TEXT")
            logger.info("Added custom_headers column to reverse_proxies table")
        except sqlite3.OperationalError:
            pass

        try:
            cursor.execute("ALTER TABLE reverse_proxies ADD COLUMN load_balance TEXT")
            logger.info("Added load_balance column to reverse_proxies table")
        except sqlite3.OperationalError:
            pass
...
async def broadcast_status_update(item_type: str, item_id: str, online: bool):
    ...
    message = f"data: {json.dumps({'type': item_type, 'id': item_id, 'online': online})}\n\n"
    ...
    while True:
        try:
            ...
            for proxy in proxies:
                ...
                if not upstream:
                    logger.warning(f"Background health check for proxy {proxy_id}: no upstream found, skipping")
                    continue
...
def build_caddy_config():
    try:
        ...
        for proxy in proxies:
            ...
            if advanced_config:
                # Advanced config exists but is not routes array format
                # Could be full Caddy JSON format or custom handler JSON
...
async def get_wifi_password_status(session_id: Optional[str] = Cookie(None)):
    ...
    # Check if RADIUS is enabled (works with all EAP methods: PAP, PEAP, EAP-TTLS)
    settings = get_settings_from_db()
    ...
    return {
        ...
        "is_eligible": radius_enabled and is_in_radius_group,
        ...
        try:
            ...
            for proxy in proxies:
                if proxy.get('enabled', False):
                    ...
                    yield f"data: {json.dumps({'type': 'proxy', 'id': proxy_id, 'online': online})}\n\n"
            ...
            for website in websites:
                if website.get('enabled', False):
                    ...
                    yield f"data: {json.dumps({'type': 'website', 'id': website_id, 'online': online})}\n\n"
